...
-------------------------------------------------------------------------
  ...
  >>>>>>>>>>>>>>>>>>>>  Blocks 8.0 <<<<<<<<<<<<<<<<<<<<<<<<<<<<
  12/16/94 Changed aachar_to_num() to accept lower case letters.
   8/ 1/95 Changed get_ids() to check for BLOCK in .lis file
  >>>>>>>>>>>>>>>>>>>>  Blocks 9.0 <<<<<<<<<<<<<<<<<<<<<<<<<<<<
   7/ 7/97 Changed check_entry() to look for seq. id to be extracted
           anywhere on title line.
   7/23/97 Added database type PROCLASS to init_dbs()
   2/26/98 Changed get_ids() to mark sequences as fragments if PS=P in .lis
=========================================================================*/
/*DOS #include <ctype.h>
#include <io.h>
#include <dir.h>
...
struct db_info *dbs[];
{
   ...
   dbs[PROC]->type = "PROCLASS";	/* PCFam, not PCSeq  */
   dbs[PROC]->start = "PCF_AC";
   dbs[PROC]->desc = "PCF_DE";
   dbs[PROC]->seq = "PCS_ID";
   dbs[PROC]->end = "//";
   dbs[PROC]->title_offset = 12;
   dbs[PROC]->seq_offset = 12;
...
*/
	 id = check_entry(ids, &line[dbs[db]->title_offset]);
...
*/
      /*   Look for id anywhere in the entry, assume it is unique */
      if (strstr(entry, id->entry) != NULL && id->found == NO)
	      return(id);
      id = id->next;
   }
   return(NULL);

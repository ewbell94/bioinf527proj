...
#define MAXNSEQ 30000		/*  UNIX MAXSEQS */
#define AAS 20
...
for (row=0; row<AAS; row++)
{
   ...
   for (col=0; col<=row; col++)
   {
    ...
    if (oij > 0.000000001) s = log( (double) oij);
	 ...
	 /*--- rounda off for the log base 10 matrix ----*/
	 dtemp = 10.0 * s / log (10.0);	/*-- log base 10 */
    tij[row][col] = rounda(dtemp);
...
xx /= (double) (AaFreq[5] + AaFreq[6]);   xx /= (double) totaas;
   fprintf(fout, "%3d ", rounda(x) );     /* XB */
   fprintf(fout, "%3d ", rounda(xx) );    /* XZ */
   fprintf(fout, "%3d\n", rounda(dtemp1) );      /*  XX */
fclose(fout);
/*   for (col=0; col<AAS; col++)  printf(" %1s  ", num_to_aachar(col));
   printf(" B   Z   X");  */
